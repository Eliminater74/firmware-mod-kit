...
static int find_identical_file(struct entry* orig, struct entry* newfile)
{
    ...
    return (find_identical_file(orig->child, newfile) || find_identical_file(orig->next, newfile));
}
...
static unsigned int parse_directory(struct entry* root_entry, const char* name, struct entry** prev, loff_t* fslen_ub)
{
    ...
    *endpath = '/';
    ...
    dircount = scandir(name, &dirlist, NULL, (int (*)(const struct dirent**, const struct dirent**))cramsort);
    ...
    for (dirindex = 0; dirindex < dircount; dirindex++) {
        ...
        entry->name = (unsigned char*)strdup((const char*)dirent->d_name);
        if (!entry->name) {
...
static int is_zero(char const* begin, unsigned len)
{
    ...
    return (len-- == 0 || (begin[0] == '\0' && (len-- == 0 || (begin[1] == '\0' && (len-- == 0 || (begin[2] == '\0' && (len-- == 0 || (begin[3] == '\0' && memcmp(begin, begin + 4, len) == 0))))))));
}
...
int main(int argc, char** argv)
{
    ...
    crc = crc32(crc, (const Bytef*)(rom_image + opt_pad), (unsigned int)(offset - opt_pad));
    ...
    if (warn_dev)
        fprintf(stderr,
            ...
            CRAMFS_OFFSET_WIDTH);
    if (opt_errors && (warn_namelen || warn_skip || warn_size || warn_uid || warn_gid || warn_dev))
        exit(MKFS_ERROR);
