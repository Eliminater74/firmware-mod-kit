...
void add_old_root_entry(char* name, squashfs_inode_t inode, int inode_number, int type);
...
extern long long read_filesystem(char* root_name, int fd, squashfs_super_block* sBlk, char** cinode_table,
    ...
    void(push_directory_entry)(char*, squashfs_inode_t, int, int),
    squashfs_fragment_entry** fragment_table);
int get_sorted_inode(squashfs_inode_t* inode, struct stat* buf);
...
#define MKINODE(A) ((squashfs_inode_t)(((squashfs_inode_t)inode_bytes << 16) + (((char*)A) - data_cache)))
...
long long write_directories()
{
    ...
    while (directory_cache_bytes) {
        if (directory_size - directory_bytes < ((SQUASHFS_METADATA_SIZE << 1) + 2)) {
            if ((directory_table = (char*)realloc(directory_table, directory_size + ((SQUASHFS_METADATA_SIZE << 1) + 2))) == NULL) {
...
int create_inode(squashfs_inode_t* i_no, struct dir_ent* dir_ent, int type, long long byte_size, long long start_block, unsigned int offset, unsigned int* block_list, struct fragment* fragment, struct directory* dir_in)
{
    ...
    if (type == SQUASHFS_FILE_TYPE) {
        ...
        dev->nlink = nlink;
        dev->rdev = (unsigned short)((major(buf->st_rdev) << 8) | (minor(buf->st_rdev) & 0xff));
        if (!swap)
...
void add_dir(squashfs_inode_t inode, unsigned int inode_number, char* name, int type, struct directory* dir)
{
    unsigned char* buff;
...
int write_file(squashfs_inode_t* inode, struct dir_ent* dir_ent, long long size, int* duplicate_file)
{
...
void dir_scan(squashfs_inode_t* inode, char* pathname, int (*_readdir)(char*, char*, struct dir_info*))
{
...
void add_old_root_entry(char* name, squashfs_inode_t inode, int inode_number, int type)
{
    if ((old_root_entry = (struct old_root_entry_info*)realloc(old_root_entry, sizeof(struct old_root_entry_info) * (old_root_entries + 1))) == NULL)
...
/* version of the signature? doesn't matter, I fix it here. */
    ...
    if (delete) {
        ...
        if ((bytes = read_filesystem(root_name, fd, &sBlk, &inode_table, &data_cache,
                 ...
                 &inode_dir_parent_inode, add_old_root_entry, &fragment_table))
            == 0) {
            ERROR("Failed to read existing filesystem - will not overwrite - ABORTING!\n");
            EXIT_MKSQUASHFS();
        }
...
restore_filesystem:
    ...
    total_bytes += total_inode_bytes + total_directory_bytes + uid_count * sizeof(unsigned short) + guid_count * sizeof(unsigned short) + sizeof(squashfs_super_block);

    printf("\n%s filesystem, data block size %d, %s data, %s metadata, %s fragments\n", be ? "Big endian" : "Little endian", block_size, noD ? "uncompressed" : "compressed", noI ? "uncompressed" : "compressed", no_fragments ? "no" : noF ? "uncompressed"
                                                                                                                                                                                                                                             : "compressed");
