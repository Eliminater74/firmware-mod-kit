...
int main(int argc, char* argv[])
{
    ...
    if (delete) {
        ...
        unsigned root_inode_start = SQUASHFS_INODE_BLK(sBlk.root_inode), root_inode_offset = SQUASHFS_INODE_OFFSET(sBlk.root_inode);
        ...
        if ((bytes = read_filesystem(root_name, fd, &sBlk, &inode_table, &data_cache,
                 ...
                 &total_bytes, &total_inode_bytes, &total_directory_bytes, add_old_root_entry, &fragment_table))
            == 0) {
